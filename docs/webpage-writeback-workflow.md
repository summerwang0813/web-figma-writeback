...
3. 颜色必须引用组件库变量；没有变量时先建立 variables 后再绑定。已有变量但缺少某个颜色时，新增颜色变量名称必须以 `*` 结尾，再绑定到节点。
...
优先搜索或引用组件库中的变量。没有任何变量时，先在当前文件创建 fallback 变量集合，再写页面；已有变量但缺少需要的颜色时，只新增缺失颜色，新增变量名后面加 `*`，例如 `text/price*`、`border/active*`。

...
- Writes select/dropdown chevrons as vector/SVG icons instead of fragile text glyphs.
- Preserves ecommerce state semantics: unselected rows stay plain unless the webpage renders a label, order status badges use distinct semantic state colors, and order progress writes back as a global timeline/stepper instead of button-like pills.
